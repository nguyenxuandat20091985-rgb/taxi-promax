...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!uid || !days) {
      console.warn(`[webhook] skip order=${orderCode} plan=${plan} uid=${uid}`);
    ...
    try {
      ...
      if (d && d.tp_expiry && parseInt(d.tp_expiry, 10) > base) base = parseInt(d.tp_expiry, 10);
    } catch (e) {}

    const newExpiry = base + days * 24 * 60 * 60 * 1000;
    await fetch(`${FIREBASE_URL}/drivers/${encodeURIComponent(uid)}.json`, {
      method: 'PATCH',
