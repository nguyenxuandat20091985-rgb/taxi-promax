/**
 * api/admin-login.js (bản đã loại bỏ hard-code credential)
 *
 * Thay thế file api/admin-login.js hiện tại.
 * Bắt buộc set 3 biến môi trường trên Vercel:
 *   ADMIN_PHONE
 *   ADMIN_PASSWORD_HASH   (sha256 hex của mật khẩu)
 *   ADMIN_SESSION_SECRET  (chuỗi dài ≥ 32 ký tự)
 *
 * Không còn DEFAULT_ADMIN_PHONE / DEFAULT_ADMIN_PASSWORD trong code.
 */
...
export default async function handler(req, res) {
  ...
  if (!configuredPhone || !configuredHash) {
    console.error('[admin-login] ADMIN_PHONE or ADMIN_PASSWORD_HASH missing');
    return res.status(503).json({
      success: false,
      error: 'Admin login is not configured. Set ADMIN_PHONE and ADMIN_PASSWORD_HASH.'
    });
  }

  if (sessionSecret === DEFAULT_ADMIN_SESSION_SECRET) {
    console.warn('[admin-login] Using fallback ADMIN_SESSION_SECRET — change in production!');
  }
