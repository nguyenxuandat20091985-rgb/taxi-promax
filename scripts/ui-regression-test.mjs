...
const locationCore = read('js/location-core.js');
const ev = read('js/driver/06-ev.js');
const careAI = read('js/modules/promax-care-ai.js');
const careApi = read('api/ai-care.js');
...
assert.match(html, /<script src="js\/driver\/00-core-runtime\.js\?v=20260828-v61"><\/script>/);
...
assert.match(html, /28-gps-guide\.js\?v=20260828-gps-owner1/);
...
assert.doesNotMatch(gpsDual, /navigator\.geolocation\.(getCurrentPosition|watchPosition)\s*\(/);
assert.doesNotMatch(locationCore, /navigator\.geolocation\.watchPosition\s*\(/);
assert.doesNotMatch(ev, /navigator\.geolocation\.watchPosition\s*\(/);
...
assert.match(html, /js\/driver\/25-gps-dual\.js\?v=20260828-gps-owner1/);
assert.match(html, /js\/modules\/gps-boost\.js/);
assert.match(html, /js\/modules\/driver-dashboard\.js\?v=20260828-dashboard2/);
assert.match(html, /js\/modules\/ops-stability\.js\?v=20260828-ops1/);
assert.match(html, /js\/modules\/ai-registry\.js\?v=20260828-ai-registry1/);
assert.match(html, /js\/modules\/promax-care-ai\.js\?v=20260828-ai-care1/);
assert.match(aiRegistry, /claim/);
assert.match(aiRegistry, /PromaxAIRegistry/);
assert.match(aiCopilot, /single owner/);
assert.doesNotMatch(aiCopilot, /navigator\.geolocation\.watchPosition\s*\(/);
assert.match(careAI, /\/api\/ai-care/);
assert.match(careAI, /Care AI hỗ trợ/);
assert.match(careApi, /module\.exports|export default|handler/);
assert.match(html, /Gói thuê bao tháng|driver-dashboard/);
assert.doesNotMatch(html, /js\/\\u200bops-stability\.js/);
