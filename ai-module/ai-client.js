// =========================================================
// TAXI PROMAX AI - THƯ KÝ 3D NGƯỜI THẬT (V19 - CHỐT HẠ)
// KHÔNG LỖI HÌNH - KHÔNG TÊN RIÊNG - NÃO GPT-4O
// =========================================================
...
(function() {
    ...
    style.innerHTML = `
        ...
        /* Icon Thư Ký 3D Xinh Đẹp - Link siêu bền */
        #ai-root { 
            width: 85px; height: 85px; border-radius: 50%; 
            background: url('https://lh3.googleusercontent.com/d/1vS8u7v0V4L7xT_nB4N_pL6Tj_E7nUu-Y') no-repeat center;
        ...
        #ai-chat-box { 
            ...
            border: 2px solid #ff80ab; overflow: hidden; animation: bounceIn 0.5s;
        }
        @keyframes bounceIn { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
    ...
    wrapper.innerHTML = `<div id="ai-chat-box"><div class="ai-header">💖 THƯ KÝ TAXI PROMAX</div><div id="ai-content"></div><div class="ai-input-area"><button id="ai-mic">🎤</button><input type="text" id="ai-txt" placeholder="Nói với em đi anh..."><button id="ai-send">🚀</button></div></div><div id="ai-root"></div>`;
    ...
    async function getAIResponse(userInput) {
        try {
            const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=AIzaSyBYIpmslXFTkETW7cfiPeLJ0oPcgMJUn2g`, {
                ...
                body: JSON.stringify({ contents: [{ parts: [{ text: `Bạn là cô thư ký xinh đẹp của Taxi Promax. KHÔNG ĐƯỢC gọi tên Đạt. Chỉ gọi 'anh' xưng 'em'. Trả lời ngọt ngào, ngắn gọn. Câu hỏi: ${userInput}` }] }] })
            });
            const data = await res.json();
            return data.candidates[0].content.parts[0].text;
        } catch (e) { return "Em luôn ở bên anh đây! ❤️"; }
    }
    ...
    wrapper.ontouchmove = (e) => { drag = true; x = e.touches[0].clientX - sx; y = e.touches[0].clientY - sy; wrapper.style.transform = `translate3d(${x}px, ${y}px, 0)`; e.preventDefault(); };
