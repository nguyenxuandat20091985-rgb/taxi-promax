/* ========== 🚨 SOS CỘNG ĐỒNG v2 - ghi âm + vị trí + giám sát admin ========== */
(function(){
    'use strict';
    var ADMIN_PHONE = '0388724966';
    var sosState = { code: null, recEnd: 0, cdTimer: null, liveTimer: null };

    /* ===== CSS ===== */
    var css = document.createElement('style');
    css.textContent =
        '.sos-overlay{position:fixed;inset:0;background:linear-gradient(135deg,#7f1d1d,#d32f2f);z-index:19000;display:none;flex-direction:column;align-items:center;justify-content:center;color:#fff;padding:24px;text-align:center;}' +
        '.sos-overlay.show{display:flex;}' +
        '.sos-pulse{font-size:64px;animation:sosPulse 1.2s infinite;}' +
        '@keyframes sosPulse{0%,100%{transform:scale(1);}50%{transform:scale(1.15);}}' +
        '.sos-btn{width:100%;max-width:320px;margin-top:10px;padding:14px;border:none;border-radius:14px;font-size:15px;font-weight:800;cursor:pointer;}' +
